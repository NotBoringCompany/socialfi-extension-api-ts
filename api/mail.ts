...
export const createMail = async (
    ...
    try {
        ...
        const finalReceiverIds =
            receivers === 'all' ? (await UserModel.find().lean()).map((user) => user._id) : receiverIds;
...
export const getAllUserMails = async (twitterId: string, page: number, limit: number): Promise<ReturnValue> => {
    ...
    try {
        ...
        const mailReceiverData = await MailReceiverDataModel.find({
            userId: user._id,
            'deletedStatus.status': false,
        }).lean();

        // fetch total mail count for pagination calculation
        const totalCount = await MailModel.countDocuments({
            _id: { $in: mailReceiverData.map((mail) => mail.mailId) },
        });

        // fetch the mails using the mail IDs
        const mailIds = mailReceiverData.map((mail) => mail.mailId);

        const mails = await MailModel.find({ _id: { $in: mailIds } })
...
export const deleteMail = async (mailId: string, twitterId: string): Promise<ReturnValue> => {
    try {
        ...
        await MailReceiverDataModel.updateOne(
            { mailId, userId },
            {
                $set: {
    ...
    try {
        ...
        for (const attachment of mailData.attachments) {
            ...
            if (type === 'food') {
                ...
                const existingFoodIndex = (user.inventory?.foods as Food[]).findIndex(
                    (food) => food.type === attachment.name
                );
                ...
                const existingItemIndex = (user.inventory?.items as Item[]).findIndex(
                    (item) => item.type === attachment.name
                );
                ...
                const existingResourceIndex = (user.inventory?.resources as ExtendedResource[]).findIndex(
                    (resource) => resource.type === attachment.name
                );

                // if the resource already exists, increment the amount
                if (existingResourceIndex !== -1) {
                    userUpdateOperations.$inc[`inventory.resources.${existingResourceIndex}.amount`] =
                        attachment.amount;
                } else {
...
export const readAndClaimAllMails = async (twitterId: string): Promise<ReturnValue> => {
    try {
        ...
        const mailReceiverData = await MailReceiverDataModel.find({
            userId,
            $or: [{ 'claimedStatus.status': false }, { 'deletedStatus.status': false }],
        }).lean();
            ...
            if (mail.attachments.length === 0) {
                ...
                for (const attachment of mail.attachments) {
                    ...
                    if (type === 'food') {
                        ...
                        const existingFoodIndexInOps = userUpdateOperations.$push['inventory.foods'].$each.findIndex(
                            (food: Food) => food.type === attachment.name
                        );
                        ...
                        if (existingFoodIndexInOps !== -1) {
                            userUpdateOperations.$push['inventory.foods'].$each[existingFoodIndexInOps].amount +=
                                attachment.amount;
                        } else {
                            ...
                            if (existingFoodIndex !== -1) {
                                userUpdateOperations.$inc[`inventory.foods.${existingFoodIndex}.amount`] =
                                    attachment.amount;
                            } else {
                        ...
                        const existingItemIndexInOps = userUpdateOperations.$push['inventory.items'].$each.findIndex(
                            (item: Item) => item.type === attachment.name
                        );
                        ...
                        if (existingItemIndexInOps !== -1) {
                            userUpdateOperations.$push['inventory.items'].$each[existingItemIndexInOps].amount +=
                                attachment.amount;
                        } else {
                            ...
                            if (existingItemIndex !== -1) {
                                userUpdateOperations.$inc[`inventory.items.${existingItemIndex}.amount`] =
                                    attachment.amount;
                            } else {
                        ...
                        const existingResourceIndexInOps = userUpdateOperations.$push[
                            'inventory.resources'
                        ].$each.findIndex((resource: ExtendedResource) => resource.type === attachment.name);
        ...
        return {
            status: Status.SUCCESS,
            message:
                '(readAndClaimAllMails) Successfully claimed and read all mails and added rewards to user inventory.',
            data: {
                claimedAttachments: claimedAttachments,
            },
        };
...
export const deleteAllReadAndClaimedMails = async (twitterId: string): Promise<ReturnValue> => {
    try {
        ...
        await MailReceiverDataModel.updateMany(
            { mailId: { $in: mailIdsToDelete }, userId },
            {
                $set: {
                    'deletedStatus.status': true,
                    'deletedStatus.timestamp': Math.floor(Date.now() / 1000),
...
export const purgeExpiredMails = async (): Promise<void> => {
    try {
        ...
        await MailReceiverDataModel.deleteMany({
            mailId: { $nin: (await MailModel.find().lean()).map((mail) => mail._id) },
        });
