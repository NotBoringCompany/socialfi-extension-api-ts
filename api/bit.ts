  ...
  try {
    ...
    if (placedIslandId !== 0) {
      ...
      const gatheringRateModifiers =
        islandStatsModifiers?.gatheringRateModifiers as Modifier[];
      const earningRateModifiers =
        islandStatsModifiers?.earningRateModifiers as Modifier[];
      const resourceCapModifiers =
        islandStatsModifiers?.resourceCapModifiers as Modifier[];
      ...
      const gatheringRateModifierIndex = gatheringRateModifiers.findIndex(
        (modifier: Modifier) => modifier.origin.includes(`Bit ID #${bitId}`)
      );
      const earningRateModifierIndex = earningRateModifiers.findIndex(
        (modifier: Modifier) => modifier.origin.includes(`Bit ID #${bitId}`)
      );
      const resourceCapModifierIndex = resourceCapModifiers.findIndex(
        (modifier: Modifier) => modifier.origin.includes(`Bit ID #${bitId}`)
      );
      ...
      if (otherBitIds.length > 0) {
        ...
        otherBits.forEach((otherBit) => {
          ...
          const gatheringRateModifiers =
            otherBitStatsModifiers?.gatheringRateModifiers as Modifier[];
          const earningRateModifiers =
            otherBitStatsModifiers?.earningRateModifiers as Modifier[];
          const energyRateModifiers =
            otherBitStatsModifiers?.energyRateModifiers as Modifier[];
          const foodConsumptionEfficiencyModifiers =
            otherBitStatsModifiers?.foodConsumptionEfficiencyModifiers as Modifier[];
          ...
          const energyRateModifierIndex = energyRateModifiers.findIndex(
            (modifier: Modifier) => modifier.origin.includes(`Bit ID #${bitId}`)
          );
          const foodConsumptionEfficiencyModifierIndex =
            foodConsumptionEfficiencyModifiers.findIndex((modifier: Modifier) =>
              modifier.origin.includes(`Bit ID #${bitId}`)
            );
              ...
              updateOperations: {
                $pull: {
                  "bitStatsModifiers.energyRateModifiers":
                    energyRateModifiers[energyRateModifierIndex],
                },
                $inc: {},
                $set: {},
                $push: {},
          ...
          if (foodConsumptionEfficiencyModifierIndex !== -1) {
            bitUpdateOperations.push({
              ...
              updateOperations: {
                $pull: {
                  "bitStatsModifiers.foodConsumptionEfficiencyModifiers":
                    foodConsumptionEfficiencyModifiers[
                      foodConsumptionEfficiencyModifierIndex
                    ],
                },
                $inc: {},
                $set: {},
    ...
    const bitUpdatePromises =
      bitUpdateOperations.length > 0 &&
      bitUpdateOperations.map(async (op) => {
...
export const feedBit = async (
  ...
  try {
    ...
    const foodConsumptionModifiers = bit.bitStatsModifiers
      ?.foodConsumptionEfficiencyModifiers as Modifier[];
    const foodConsumptionMultiplier = foodConsumptionModifiers.reduce(
    ...
    if (gatheringRateModifierIndex !== -1) {
      ...
      if (gatheringRateModifier.value === 1) {
        bitUpdateOperations.$pull["bitStatsModifiers.gatheringRateModifiers"] =
          { origin: "Energy Threshold Reduction" };
      } else {
        bitUpdateOperations.$set[
          `bitStatsModifiers.gatheringRateModifiers.$[elem].value`
        ] = gatheringRateModifier.value;
      }
    } else {
      bitUpdateOperations.$push["bitStatsModifiers.gatheringRateModifiers"] =
    ...
    if (earningRateModifierIndex !== -1) {
      ...
      if (earningRateModifier.value === 1) {
        bitUpdateOperations.$pull["bitStatsModifiers.earningRateModifiers"] = {
          origin: "Energy Threshold Reduction",
        };
      } else {
        bitUpdateOperations.$set[
      ...
      bitUpdateOperations.$push["bitStatsModifiers.earningRateModifiers"] =
        earningRateModifier;
    }
  ...
  try {
    ...
    const bulkWriteOperations = bits
      .map((bit) => {
        ...
        const baseDepletionRate =
          bit.farmingStats?.currentEnergyDepletionRate / 6;

        // get (if applicable) the bit's energy rate modifiers. multiply the value to get the final depletion rate
        const energyRateModifiers = bit.bitStatsModifiers
          ?.energyRateModifiers as Modifier[];
        const energyRateMultiplier = energyRateModifiers.reduce(
          (acc, modifier) => acc * modifier.value,
          1
        );
...
export const evolveBit = async (
  ...
  try {
    ...
    if (bit.premium) {
      ...
      ).find((resource) => resource.type === BarrenResource.SEAWEED);
      ...
      const userSeaweedIndex = (
        user.inventory?.resources as ExtendedResource[]
      ).findIndex((resource) => resource.type === BarrenResource.SEAWEED);
      // deduct the required seaweed from the user's inventory
      userUpdateOperations.$inc[
        `inventory.resources.${userSeaweedIndex}.amount`
      ] = -requiredSeaweed;
...
export const randomizeFarmingStats = (rarity: BitRarity): BitFarmingStats => {
  ...
  const gatheringRateGrowth = defaultGatheringRateGrowth * rand1;
  ...
  const currentEnergyDepletionRate = baseEnergyDepletionRate;
