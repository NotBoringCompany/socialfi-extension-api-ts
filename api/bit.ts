...
export const feedBit = async (twitterId: string, bitId: number, foodType: FoodType): Promise<ReturnValue> => {
    ...
    try {
        ...
        if (!user.inventory.bitIds.includes(bitId)) {
            return { status: Status.ERROR, message: `(feedBit) User does not own the bit.` };
        }
        // check if user has the food
        const userFood = user.inventory.foods.find((food: Food) => food.type === foodType);
        ...
        if (bit.farmingStats?.currentEnergy === 100) {
            return { status: Status.ERROR, message: `(feedBit) Bit's energy is already at max.` };
        }
        ...
        const actualToReplenish = Math.min(toReplenish, 100 - bit.farmingStats.currentEnergy);
        ...
        if (gatheringRateModifierIndex !== -1) {
            ...
            if (gatheringRateModifier.value === 1) {
                ...
                Object.assign(bitUpdateOperations, {
                    $set: { 'bitStatsModifiers.gatheringRateModifiers.$[elem].value': gatheringRateModifier.value }
                });
            }
        } else {
            Object.assign(bitUpdateOperations, {
        ...
        if (earningRateModifierIndex !== -1) {
            ...
            if (earningRateModifier.value === 1) {
                ...
                Object.assign(bitUpdateOperations, {
                    $set: { 'bitStatsModifiers.earningRateModifiers.$[elem].value': earningRateModifier.value }
                });
            }
        } else {
            Object.assign(bitUpdateOperations, {
        ...
        if (bit.placedIslandId !== 0) {
            const island = await Island.findOne({ islandId: bit.placedIslandId }).lean();
            ...
            if (island.islandEarningStats?.totalXCookiesSpent === 0) {
                Object.assign(islandUpdateOperations, {
                    $inc: { 'islandEarningStats.totalXCookiesSpent': foodCost, 'islandEarningStats.earningStart': Math.floor(Date.now() / 1000) }
                });
            } else {
        ...
        await Promise.all([
            ...
            Bit.updateOne({ bitId }, bitUpdateOperations),
            Island.updateOne({ islandId: bit.placedIslandId }, islandUpdateOperations)
        ]);
...
export const depleteEnergy = async (): Promise<void> => {
    ...
    try {
        ...
        const bulkWriteOperations = bits.map(bit => {
            ...
            console.log(`(depleteEnergy) Bit ${bit.bitId} - current energy is less than 0? ${currentEnergy <= 0} - depletion rate: ${depletionRate}`);
            ...
            if (gatheringRateModifierIndex !== -1) {
                ...
                if (gatheringRateModifier.value === 1) {
                    console.log(`Bit ID ${bit.bitId} - gathering rate modifier exists AND value is 1. updating energy and removing modifier`);
                ...
                // if the new gathering rate modifier is not 1, update it
                    ...
                    console.log(`Bit ID ${bit.bitId} - gathering rate modifier exists AND value is not 1. updating energy and modifier`);
            ...
            // if the modifier doesn't exist, push it
                ...
                if (gatheringRateModifier.value === 1) {
                    console.log(`Bit ID ${bit.bitId} - gathering rate modifier does not exist AND value is 1. updating energy and NOT pushing modifier.`);
                ...
                // if the new gathering rate modifier is not 1, push the modifier
                    ...
                    console.log(`Bit ID ${bit.bitId} - gathering rate modifier does not exist AND value is not 1. updating energy and pushing modifier.`);
            ...
            if (earningRateModifierIndex !== -1) {
                ...
                if (earningRateModifier.value === 1) {
                    console.log(`Bit ID ${bit.bitId} - earning rate modifier exists AND value is 1. removing modifier`);
                ...
                // if the new earning rate modifier is not 1, update it
                    ...
                    console.log(`Bit ID ${bit.bitId} - earning rate modifier exists AND value is not 1. updating modifier`);
            ...
            // if the modifier doesn't exist, push it
                ...
                if (earningRateModifier.value !== 1) {
                    console.log(`Bit ID ${bit.bitId} - earning rate modifier does not exist AND value is not 1. pushing modifier`);
                    ...
                    console.log(`Bit ID ${bit.bitId} - earning rate modifier does not exist AND value is 1. doing nothing.`);
                }
            }
