...
import {
  BARREN_ISLE_COMMON_DROP_CHANCE,
  BASE_CARESS_PER_TAPPING,
  BASE_ENERGY_PER_TAPPING,
  BIT_PLACEMENT_CAP,
  BIT_PLACEMENT_MIN_RARITY_REQUIREMENT,
  DAILY_BONUS_RESOURCES_GATHERABLE,
  DEFAULT_RESOURCE_CAP,
  EARNING_RATE_REDUCTION_MODIFIER,
  GATHERING_RATE_REDUCTION_MODIFIER,
  ISLAND_EVOLUTION_COST,
  ISLAND_RARITY_DEVIATION_MODIFIERS,
  ISLAND_TAPPING_MILESTONE_LIMIT,
  ISLAND_TAPPING_REQUIREMENT,
  MAX_ISLAND_LEVEL,
  RARITY_DEVIATION_REDUCTIONS,
  RESOURCES_CLAIM_COOLDOWN,
  RESOURCE_DROP_CHANCES,
  RESOURCE_DROP_CHANCES_LEVEL_DIFF,
  TOTAL_ACTIVE_ISLANDS_ALLOWED,
  X_COOKIE_CLAIM_COOLDOWN,
  X_COOKIE_TAX,
  randomizeIslandTraits,
} from '../utils/constants/island';
import { calcBitCurrentRate, getBits } from './bit';
import {
  BarrenResource,
  ExtendedResource,
  ExtendedResourceOrigin,
  Resource,
  ResourceLine,
  ResourceRarity,
  ResourceRarityNumeric,
  ResourceType,
  SimplifiedResource,
    ...
    bits.forEach((bit) => {
      ...
      if (
        bitTraits.some((traitData) => {
          return (
            traitData.trait === BitTrait.INFLUENTIAL ||
        ...
        const gatheringRateModifier: Modifier = {
          origin: `Bit ID #${bit.bitId}'s Trait: ${
            bitTraits.some(
              (traitData) => traitData.trait === BitTrait.INFLUENTIAL
            )
              ? 'Influential'
              : bitTraits.some(
                  (traitData) => traitData.trait === BitTrait.FAMOUS
                )
              ? 'Famous'
              : bitTraits.some(
                  (traitData) => traitData.trait === BitTrait.MANNERLESS
                )
              ? 'Mannerless'
              : 'Antagonistic'
          }`,
          value: bitTraits.some(
            (traitData) => traitData.trait === BitTrait.INFLUENTIAL
          )
            ? 1.01
            : bitTraits.some((traitData) => traitData.trait === BitTrait.FAMOUS)
            ? 1.005
            : bitTraits.some(
                (traitData) => traitData.trait === BitTrait.MANNERLESS
              )
 ...
 */
export const removeIsland = async (
  twitterId: string,
  islandId: number
): Promise<ReturnValue> => {
  try {
 ...
 */
export const evolveIsland = async (
  twitterId: string,
  islandId: number,
  choice: 'xCookies' | 'Cookie Crumbs'
): Promise<ReturnValue> => {
  try {
    const [user, island] = await Promise.all([
      UserModel.findOne({ twitterId }).lean(),
    ...
    if (choice === 'xCookies') {
      ...
      const { xCookies: requiredXCookies } = ISLAND_EVOLUTION_COST(
      ...
      userUpdateOperations.$inc['inventory.xCookieData.currentXCookies'] =
        -requiredXCookies;
      userUpdateOperations.$inc['inventory.xCookieData.totalXCookiesSpent'] =
        requiredXCookies;
      userUpdateOperations.$inc['inventory.xCookieData.weeklyXCookiesSpent'] =
        requiredXCookies;
      ...
      const totalXCookiesEarnableIsZero =
        island.islandEarningStats?.totalXCookiesEarnable === 0;
      ...
      if (totalXCookiesEarnableIsZero) {
        ...
        islandUpdateOperations.$inc['islandEarningStats.totalXCookiesSpent'] =
          requiredXCookies;
        ...
        islandUpdateOperations.$inc['islandEarningStats.totalXCookiesSpent'] =
          requiredXCookies;
        islandUpdateOperations.$inc[
          'islandEarningStats.totalXCookiesEarnable'
        ] = requiredXCookies;
      }
      ...
      // calculate the cost to evolve the island based on its current level
      const { cookieCrumbs: requiredCookieCrumbs } = ISLAND_EVOLUTION_COST(
        <IslandType>island.type,
        island.currentLevel
      );
      ...
      userUpdateOperations.$inc['inventory.cookieCrumbs'] =
        -requiredCookieCrumbs;
      ...
      islandUpdateOperations.$inc['islandEarningStats.totalCookieCrumbsSpent'] =
        requiredCookieCrumbs;
      islandUpdateOperations.$inc['currentLevel'] = 1;
    }
...
export const placeBit = async (
  ...
  try {
    ...
    if (bit.placedIslandId !== 0) {
      ...
      if (
        bit.lastRelocationTimestamp + RELOCATION_COOLDOWN <
        Math.floor(Date.now() / 1000)
      ) {
        // get the previous island ID from the bit
        const prevIslandId = bit.placedIslandId;

        const prevIsland = await IslandModel.findOne({
          islandId: prevIslandId,
        }).lean();
        ...
        const prevIslandBits = await BitModel.find({
          bitId: { $in: prevIsland.placedBitIds },
        }).lean();
        ...
        for (const prevIslandBit of prevIslandBits) {
          ...
          const {
            ...
            earningRateModifiers,
            energyRateModifiers,
          }: BitStatsModifiers = prevIslandBit.bitStatsModifiers;
        ...
        const {
          ...
          gatheringRateModifiers,
          earningRateModifiers,
        }: IslandStatsModifiers = prevIsland.islandStatsModifiers;

        for (const modifier of resourceCapModifiers) {
          if (modifier.origin.includes(`Bit ID #${bit.bitId}`)) {
            prevIslandUpdateOperations.$pull[
              'islandStatsModifiers.resourceCapModifiers'
            ] = modifier;
          }
        }
        ...
        // execute the update operations
        const prevBitPromises = prevIslandBitsUpdateOperations.map(
          async (op) => {
            return BitModel.updateOne({ bitId: op.bitId }, op.updateOperations);
          }
        );
    ...
    const minRarityRequired = BIT_PLACEMENT_MIN_RARITY_REQUIREMENT(
      <IslandType>island.type
    );

    const bitRarityAllowed = checkBitRarityAllowed(
      bitRarity,
      minRarityRequired
    );
    ...
    const gatheringRateModifierIndex = (
      island.islandStatsModifiers?.gatheringRateModifiers as Modifier[]
    ).findIndex((modifier) => modifier.origin === 'Rarity Deviation');
    ...
    if (gatheringRateModifierIndex === -1) {
      ...
      if (newGatheringRateModifier.value !== 1) {
        ...
        islandUpdateOperations.$push[
          'islandStatsModifiers.gatheringRateModifiers'
        ] = newGatheringRateModifier;
      }
    } else {
      const currentValue =
        island.islandStatsModifiers?.gatheringRateModifiers[
          gatheringRateModifierIndex
        ].value;
      const newValue =
        currentValue - rarityDeviationReductions.gatheringRateReduction / 100;

      // reduce the value by the reduction amount
      islandUpdateOperations.$set[
    ...
    if (
      island.placedBitIds.length === 0 &&
      island.islandResourceStats.gatheringStart === 0
    ) {
      islandUpdateOperations.$set['islandResourceStats.gatheringStart'] =
        Math.floor(Date.now() / 1000);
    }
    ...
    bitUpdateOperations.$set['lastRelocationTimestamp'] = Math.floor(
      Date.now() / 1000
    );
...
export const unplaceBit = async (
  ...
  try {
    ...
    for (const trait of bitTraits) {
      const otherBits = await BitModel.find({
        bitId: { $in: island.placedBitIds },
      }).lean();
      ...
      if (
        ...
        const gatheringRateModifierIndex = (
          ...
          modifier.origin.includes(`Bit ID #${bit.bitId}`)
        );
        const earningRateModifierIndex = (
        ...
        if (earningRateModifierIndex !== -1) {
          islandUpdateOperations.$pull[
            ...
            island.islandStatsModifiers?.earningRateModifiers[
              earningRateModifierIndex
            ];
        }
        ...
        for (const otherBit of otherBits) {
          ...
          const gatheringRateModifierIndex = (
            otherBit.bitStatsModifiers?.gatheringRateModifiers as Modifier[]
          ).findIndex((modifier) =>
            modifier.origin.includes(`Bit ID #${bit.bitId}`)
          );
          const earningRateModifierIndex = (
            otherBit.bitStatsModifiers?.earningRateModifiers as Modifier[]
          ).findIndex((modifier) =>
            modifier.origin.includes(`Bit ID #${bit.bitId}`)
          );
              ...
              updateOperations: {
                $pull: {
                  'bitStatsModifiers.gatheringRateModifiers':
                    otherBit.bitStatsModifiers?.gatheringRateModifiers[
                      gatheringRateModifierIndex
                    ],
                },
                $inc: {},
              ...
              updateOperations: {
                $pull: {
                  'bitStatsModifiers.earningRateModifiers':
                    otherBit.bitStatsModifiers?.earningRateModifiers[
                      earningRateModifierIndex
                    ],
                },
                $inc: {},
...
export const updateExtendedTraitEffects = async (
  ...
  const otherBitIds = island.placedBitIds.filter(
    (placedBitId) => placedBitId !== bit.bitId
  );
  ...
  if (otherBits.length > 0) {
    ...
    for (const otherBit of otherBits) {
      ...
      for (const trait of traits) {
        ...
        if (trait.trait === BitTrait.TEAMWORKER) {
          if (
            BitRarityNumeric[bit.rarity] <= BitRarityNumeric[otherBit.rarity]
          ) {
            // add the new modifier to the bit's `gatheringRateModifiers` and `earningRateModifiers`
            const newGatheringRateModifier: Modifier = {
  ...
  console.log(
    `(updateExtendedTraitEffects) Extended trait effects updated for island ID ${island.islandId}.`
  );
};
 ...
 */
export const checkBitRarityAllowed = (
  bitRarity: BitRarity,
  minRarityRequired: BitRarity
): boolean => {
 ...
 */
export const checkCurrentTax = async (
  twitterId: string,
  islandId: number
): Promise<ReturnValue> => {
  try {
 ...
 */
export const updateGatheringProgressAndDropResource =
  async (): Promise<void> => {
    try {
      ...
      if (islands.length === 0 || !islands) {
        console.error(
          `(updateGatheringProgressAndDropResource) No islands found.`
        );
        return;
      }
      ...
      const bulkWriteOpsPromises = islands.map(async (island) => {
        ...
        if (status !== Status.SUCCESS) {
          console.error(
            `(updateGatheringProgressAndDropResource) Error For island ID ${island.islandId} from getBits: ${message}`
          );
          return;
        }
        ...
        const initialGrowthRates = bits.map(
          (bit) => bit.farmingStats.gatheringRateGrowth
        );
        const bitModifiers = bits.map(
          (bit) => bit.bitStatsModifiers.gatheringRateModifiers
        );
        ...
        const lastUpdatedGatheringProgress = island.islandResourceStats
          ?.lastUpdatedGatheringProgress as number;
        ...
        const resourcesPerHour =
          (gatheringRate / 100) * island.islandResourceStats?.baseResourceCap;
        const hoursToDropResource = 1 / resourcesPerHour;
        const gatheringProgressIncrementPerHour =
          (1 / hoursToDropResource) * 100;
        ...
        const gatheringProgressIncrement =
          (gatheringProgressIncrementPerHour / 3600) * timePassed;

        console.log(
          `(updateGatheringProgressAndDropResource) Island ID ${island.islandId} has a current gathering rate of ${gatheringRate} %/hour and a gathering progress increment of ${gatheringProgressIncrement}%/${timePassed} seconds.`
        );
        ...
        if (gatheringProgress + gatheringProgressIncrement < 100) {
          ...
          updateOperations.push({
            updateOne: {
              ...
              update: {
                $inc: {
                  'islandResourceStats.gatheringProgress':
                    gatheringProgressIncrement,
                },
                $set: {
                  // set the `lastUpdatedGatheringProgress` to the current time
                  'islandResourceStats.lastUpdatedGatheringProgress':
                    Math.floor(Date.now() / 1000),
                },
              },
            },
          ...
          console.log(
            `(updateGatheringProgressAndDropResource) Island ID ${
              island.islandId
            } has updated its gathering progress to ${
              gatheringProgress + gatheringProgressIncrement
            }.`
          );
        } else {
          ...
          if (status !== Status.SUCCESS) {
            console.error(
              `(updateGatheringProgressAndDropResource) Error For island ID ${island.islandId} from dropResource: ${message}`
            );
          }

          // calculate the remaining overflow of %
          finalGatheringProgress =
            (gatheringProgress + gatheringProgressIncrement) % 100;
          ...
          console.log(
            `(updateGatheringProgressAndDropResource) Island ID ${island.islandId} has dropped a resource and reset its gathering progress to ${finalGatheringProgress}.`
          );
        }
      ...
      if (bulkWriteOps.length === 0) {
        console.error(
          `(updateGatheringProgressAndDropResource) No islands have been updated.`
        );
        return;
      }
      ...
      console.log(
        `(updateGatheringProgressAndDropResource) All islands' gathering progresses have been updated.`
      );
    } catch (err: any) {
      // only console logging; this shouldn't stop the entire process
      console.error(
        `(updateGatheringProgressAndDropResource) Error: ${err.message}`
      );
    }
  };
...
export const updateGatheringProgressAndDropResourceAlt = async (
  ...
  try {
    ...
    const gatheringProgress = island.islandResourceStats
      ?.gatheringProgress as number;
      ...
      await IslandModel.updateOne(
        { islandId },
        {
          $set: {
            ...
            // set the `lastUpdatedGatheringProgress` to the current time
            'islandResourceStats.lastUpdatedGatheringProgress': Math.floor(
              Date.now() / 1000
            ),
          },
        }
...
export const updateClaimableXCookies = async (): Promise<void> => {
  try {
    ...
    const islands = await IslandModel.find({
      'islandEarningStats.totalXCookiesEarnable': { $gt: 0 },
    }).lean();
    ...
    const bulkWriteOpsPromises = islands.map(async (island) => {
      ...
      if (placedBitIds.length === 0) {
        console.log(
          `(updateClaimableXCookies) Island ID ${island.islandId} has no bits placed. Skipping...`
        );
        return [];
      }
      ...
      const currentEarningRate = calcIslandCurrentRate(
        ...
        bits.map(
          (bit) => bit.bitStatsModifiers.earningRateModifiers as Modifier[]
        ),
        island.islandStatsModifiers?.earningRateModifiers as Modifier[]
      );
      ...
      const claimableXCookies =
        (tenMinEarningRate / 100) *
        island.islandEarningStats?.totalXCookiesEarnable;

      console.log(
        `claimable xCookies for Island ${island.islandId} is ${claimableXCookies}.`
      );
      ...
      if (claimableXCookies === 0) {
        console.log(
          `(updateClaimableXCookies) Island ID ${island.islandId} has 0 claimable xCookies. Skipping...`
        );
        return [];
      }
      ...
      if (xCookiesEarned >= island.islandEarningStats?.totalXCookiesEarnable) {
        console.log(
          `(updateClaimableXCookies) Island ID ${island.islandId} has already earned all of its xCookies. Skipping...`
        );
        return [];
      }
      ...
      if (
        claimableXCookies + xCookiesEarned >
        island.islandEarningStats?.totalXCookiesEarnable
      ) {
        console.log(`(updateClaimableXCookies) Island ID ${
          island.islandId
        }'s claimableXCookies exceeds cookies spent.
                 adjusting... totalXCookiesEarnable: ${
                   island.islandEarningStats?.totalXCookiesEarnable
                 } - xCookiesEarned: ${xCookiesEarned} = ${
          island.islandEarningStats?.totalXCookiesEarnable - xCookiesEarned
        }.`);
        ...
        updateOperations.push({
          updateOne: {
            ...
            update: {
              ...
              $inc: {
                'islandEarningStats.totalXCookiesEarned':
                  island.islandEarningStats?.totalXCookiesEarnable -
                  xCookiesEarned,
                'islandEarningStats.claimableXCookies':
                  island.islandEarningStats?.totalXCookiesEarnable -
                  xCookiesEarned,
              },
            },
        ...
        console.log(
          `(updateClaimableXCookies) Island ID ${
            island.islandId
          } has updated its claimable xCookies to ${
            island.islandEarningStats?.claimableXCookies + claimableXCookies
          }.`
        );
    ...
    if (bulkWriteOps.length === 0) {
      console.error(
        `(updateClaimableXCookies) No bulk write operations found.`
      );
      return;
    }
    ...
    console.log(
      `(updateClaimableXCookies) All islands' claimableXCookies have been updated.`
    );
  } catch (err: any) {
...
export const applyGatheringProgressBooster = async (
  ...
  try {
    ...
    const allSameBooster = boosters.every(
      (booster) => booster === firstBooster
    );
    ...
    const allGatheringProgressBoosters = boosters.every((booster) =>
      booster.includes('Gathering Progress Booster')
    );
    ...
    const allowedAmount = [10, 25, 50, 100, 200, 300].includes(
      parseFloat(firstBooster.split(' ')[3])
    )
      ? 10
      : 1;
    ...
    const boosterIndex = (user.inventory.items as Item[]).findIndex(
      (item) => item.type === firstBooster
    );
    ...
    if (
      (user.inventory.items as Item[])[boosterIndex].amount < boosters.length
    ) {
      return {
        status: Status.ERROR,
    ...
    const normalResourcesGathered = (
      island.islandResourceStats?.resourcesGathered as ExtendedResource[]
    ).filter((resource) => resource.origin === ExtendedResourceOrigin.NORMAL);
    // add the amount of resources per `normalResourcesGathered` instance
    const normalResourcesGatheredAmount = normalResourcesGathered.reduce(
      (acc, resource) => acc + resource.amount,
      0
    );
    const resourcesLeft =
      island.islandResourceStats?.baseResourceCap -
      normalResourcesGatheredAmount;

    console.log(
      `resources left for island ${island.islandId}: `,
      resourcesLeft
    );
    ...
    if (boosterPercentage < 100) {
      ...
      if (gatheringProgress + boosterPercentage > 100) {
        ...
        if (resourcesLeft === 0) {
          console.log(
            `(applyGatheringProgressBooster) Island ID ${islandId} has no resources left to drop. Cannot apply booster.`
          );
        ...
        // calculate the remaining overflow of %
        const finalGatheringProgress =
          gatheringProgress + boosterPercentage - 100;

        // reset the gathering progress back to 0 + the remaining overflow of %
        islandUpdateOperations.$set['islandResourceStats.gatheringProgress'] =
          finalGatheringProgress;

        // deduct the boosters from the user's inventory, update `totalAmountConsumed` and `weeklyAmountConsumed`.
        userUpdateOperations.$inc[`inventory.items.${boosterIndex}.amount`] =
          -boosters.length;
        userUpdateOperations.$inc[
          `inventory.items.${boosterIndex}.totalAmountConsumed`
        ] = boosters.length;
        userUpdateOperations.$inc[
          `inventory.items.${boosterIndex}.weeklyAmountConsumed`
        ] = boosters.length;
        ...
        if (status !== Status.SUCCESS) {
          console.log(
            `(applyGatheringProgressBooster) Error from dropResource: ${message}`
          );
        ...
        islandUpdateOperations.$inc['islandResourceStats.gatheringProgress'] =
          boosterPercentage;
      ...
      const gatheringProgress = island.islandResourceStats?.gatheringProgress;
      const finalNonModuloGatheringProgress =
        gatheringProgress + boosterPercentage;
      const resourcesToDrop = Math.floor(finalNonModuloGatheringProgress / 100);

      console.log(
        `gathering progress of island ${island.islandId}: `,
        gatheringProgress
      );
      console.log(
        `final non-modulo gathering progress of island ${island.islandId}: `,
        finalNonModuloGatheringProgress
      );
      ...
      if (resourcesToDrop > resourcesLeft) {
        console.log(
          `(applyGatheringProgressBooster) Island ID ${islandId} does not have enough resources left to drop. Cannot apply booster.`
        );
      ...
      islandUpdateOperations.$set['islandResourceStats.gatheringProgress'] =
        finalNonModuloGatheringProgress % 100;
      ...
      for (let i = 0; i < resourcesToDrop; i++) {
        ...
        console.log(
          `dropped a resource for Island ${islandId} x${i + 1}. resource: ${
            data.resource
          }`
        );
        ...
        if (status !== Status.SUCCESS) {
          console.log(
            `(applyGatheringProgressBooster) Error from dropResource in loop: ${message}`
          );
        }
      }
...
export const updateClaimableCrumbs = async (): Promise<void> => {
  try {
    const islands = await IslandModel.find({
      'islandEarningStats.totalCookieCrumbsEarnable': { $gt: 0 },
    }).lean();
    ...
    const bulkWriteOpsPromises = islands.map(async (island) => {
      ...
      if (placedBitIds.length === 0) {
        console.log(
          `(updateClaimableCrumbs) Island ID ${island.islandId} has no bits placed. Skipping...`
        );
        return;
      }
      ...
      const tenMinEarningRate = currentEarningRate / 6;
      const claimableCookieCrumbs =
        (tenMinEarningRate / 100) *
        island.islandEarningStats?.totalCookieCrumbsEarnable;

      console.log(
        `claimable Cookie Crumbs for Island ${island.islandId} is ${claimableCookieCrumbs}.`
      );

      // get the current amount of cookie crumbs earned already
      const cookieCrumbsEarned =
        island.islandEarningStats?.totalCookieCrumbsEarned;

      // if the amount of `claimableCookieCrumbs` is 0, skip this island (shouldn't happen, but just in case)
      if (claimableCookieCrumbs === 0) {
        console.log(
          `(updateClaimableCrumbs) Island ID ${island.islandId} has 0 claimable Cookie Crumbs. Skipping...`
        );
        return [];
      }
      ...
      if (
        ...
        console.log(
          `(updateClaimableCrumbs) Island ID ${island.islandId} has already earned all of its Cookie Crumbs. Skipping...`
        );
        return [];
      }
      ...
      if (
        claimableCookieCrumbs + cookieCrumbsEarned >
        island.islandEarningStats?.totalCookieCrumbsEarnable
      ) {
        console.log(`(updateClaimableCrumbs) Island ID ${
          island.islandId
        }'s claimableCookieCrumbs exceeds cookie crumbs spent.
                 adjusting... totalCookieCrumbsEarnable: ${
                   island.islandEarningStats?.totalCookieCrumbsEarnable
                 } - cookieCrumbsEarned: ${cookieCrumbsEarned} = ${
          island.islandEarningStats?.totalCookieCrumbsEarnable -
          cookieCrumbsEarned
        }.`);
        ...
        updateOperations.push({
          updateOne: {
            ...
            update: {
              ...
              $inc: {
                'islandEarningStats.totalCookieCrumbsEarned':
                  island.islandEarningStats?.totalCookieCrumbsEarnable -
                  cookieCrumbsEarned,
                'islandEarningStats.claimableCookieCrumbs':
                  island.islandEarningStats?.totalCookieCrumbsEarnable -
                  cookieCrumbsEarned,
              },
            },
        ...
        console.log(
          `(updateClaimableCrumbs) Island ID ${
            island.islandId
          } has updated its claimable Cookie Crumbs to ${
            island.islandEarningStats?.claimableCookieCrumbs +
            claimableCookieCrumbs
          }.`
        );
    ...
    console.log(
      `(updateClaimableCrumbs) All islands' claimableCookieCrumbs have been updated.`
    );
  } catch (err: any) {
...
export const claimResources = async (
  ...
  try {
    ...
    const claimableResources = island.islandResourceStats
      ?.claimableResources as ExtendedResource[];
    ...
    if (claimType === 'manual') {
      ...
      for (let chosenResource of chosenResources) {
        ...
        const chosenResourceData = resources.find(
          (r) => r.type === chosenResource.type
        );

        const claimableResourceIndex = claimableResources.findIndex(
          (r) => r.type === chosenResource.type
        );
        ...
        if (
          chosenResource.amount >
          claimableResources[claimableResourceIndex].amount
        ) {
          return {
            status: Status.ERROR,
        ...
        const resourceWeight: number = resources.find(
          (r) => r.type === chosenResource.type
        )?.weight;
        ...
        const existingResourceIndex = (
          user.inventory?.resources as ExtendedResource[]
        ).findIndex((r) => r.type === chosenResource.type);

        if (existingResourceIndex !== -1) {
          userUpdateOperations.$inc[
            `inventory.resources.${existingResourceIndex}.amount`
          ] = chosenResource.amount;
        } else {
        ...
        if (
          ...
          ] = { type: chosenResource.type };
        } else {
          islandUpdateOperations.$inc[
            `islandResourceStats.claimableResources.${claimableResourceIndex}.amount`
          ] = -chosenResource.amount;
        }
      }
      ...
      if (
        currentInventoryWeight + totalWeightToClaim >
        user.inventory.maxWeight
      ) {
        return {
          status: Status.ERROR,
      ...
      if (
        ...
        for (const resource of claimableResources) {
          ...
          const existingResourceIndex = (
            user.inventory?.resources as ExtendedResource[]
          ).findIndex((r) => r.type === resource.type);
        ...
        const sortedResources = claimableResources.sort(
          (a, b) =>
            ResourceRarityNumeric[b.rarity] - ResourceRarityNumeric[a.rarity]
        );
        ...
        const maxAllowedWeight =
          user.inventory.maxWeight - currentInventoryWeight;
        ...
        for (const rarityGroup of Object.values(groupedResources)) {
          ...
          const sortedByWeight = rarityGroup.sort(
            (a, b) =>
              resources.find((r) => r.type === b.type)?.weight -
              resources.find((r) => r.type === a.type)?.weight
          );
          ...
          for (const resource of sortedByWeight) {
            ...
            if (currentWeight + totalWeight > maxAllowedWeight) {
              console.log(
                'current weight + total weight of resources exceeds max allowed weight!'
              );

              // calculate the amount of this resource we can claim based on the max allowed weight
              const amountToClaim = Math.floor(
                (maxAllowedWeight - currentWeight) / resourceWeight
              );
              ...
              console.log(
                'current weight + total weight of resources does not exceed max allowed weight!'
              );
    ...
    islandUpdateOperations.$set['islandResourceStats.lastClaimed'] =
      currentTime;

    console.log(
      `Island ${island.islandId} userUpdateOperations: `,
      userUpdateOperations
    );
    console.log(
      `Island ${island.islandId} islandUpdateOperations: `,
      islandUpdateOperations
    );
    ...
    await UserModel.updateOne(
      ...
      {
        $set:
          Object.keys(userUpdateOperations.$set).length > 0
            ? userUpdateOperations.$set
            : {},
        $inc:
          Object.keys(userUpdateOperations.$inc).length > 0
            ? userUpdateOperations.$inc
            : {},
      }
    );
    ...
    await UserModel.updateOne(
      ...
      {
        $pull:
          Object.keys(userUpdateOperations.$pull).length > 0
            ? userUpdateOperations.$pull
            : {},
        $push:
          Object.keys(userUpdateOperations.$push).length > 0
    ...
    await IslandModel.updateOne(
      ...
      {
        $set:
          Object.keys(islandUpdateOperations.$set).length > 0
            ? islandUpdateOperations.$set
            : {},
        $inc:
          Object.keys(islandUpdateOperations.$inc).length > 0
            ? islandUpdateOperations.$inc
            : {},
      }
    );
    ...
    await IslandModel.updateOne(
      ...
      {
        $pull:
          Object.keys(islandUpdateOperations.$pull).length > 0
            ? islandUpdateOperations.$pull
            : {},
        $push:
          Object.keys(islandUpdateOperations.$push).length > 0
            ? islandUpdateOperations.$push
            : {},
      }
    );
    ...
    return {
      ...
      data: {
        claimedResources:
          claimType === 'manual' ? chosenResources : claimedResources,
      },
    };
  } catch (err: any) {
    return {
...
export const updateDailyBonusResourcesGathered = async (): Promise<void> => {
  try {
    const islands = await IslandModel.find({
      'islandResourceStats.dailyBonusResourcesGathered': { $gt: 0 },
    }).lean();
    ...
    console.log(
      `(updateDailyBonusResourcesGathered) All islands' dailyBonusResourcesGathered have been reset.`
    );
  } catch (err: any) {
 ...
 */
export const claimXCookiesAndCrumbs = async (
  twitterId: string,
  islandId: number
): Promise<ReturnValue> => {
  try {
    ...
    if (xCookies > 0) {
      ...
      const { status, message, data } = await checkCurrentTax(
        twitterId,
        islandId
      );
      ...
      const islandClaimingIndex = (
        user.inventory.xCookieData.extendedXCookieData as ExtendedXCookieData[]
      ).findIndex((data) => data.source === XCookieSource.ISLAND_CLAIMING);
      ...
      if (islandClaimingIndex === -1) {
        ...
        userUpdateOperations.$inc[
          `inventory.xCookieData.extendedXCookieData.${islandClaimingIndex}.xCookies`
        ] = xCookiesAfterTax;
      }
...
export const dropResource = async (islandId: number): Promise<ReturnValue> => {
  try {
    ...
    const baseResourceCap = island.islandResourceStats
      ?.baseResourceCap as number;
    ...
    const resourcesGathered: ExtendedResource[] =
      island.islandResourceStats?.resourcesGathered.filter(
        (r: ExtendedResource) => r.origin === ExtendedResourceOrigin.NORMAL
      );
    // get the amount per `resourcesGathered` instance
    const resourcesGatheredAmount = resourcesGathered.reduce(
      (acc, r) => acc + r.amount,
      0
    );
    ...
    if (<IslandType>island.type === IslandType.BARREN) {
      const seaweedGathered = resourcesGathered.filter(
        (r) => r.type === BarrenResource.SEAWEED
      );
      if (baseResourceCap - seaweedGathered.length <= 0) {
        console.log(
          `(dropResource) No resources left to drop for Island ${islandId}.`
        );
        ...
        if (island.islandResourceStats?.gatheringEnd === 0) {
          await IslandModel.updateOne(
            ...
            {
              $set: {
                'islandResourceStats.gatheringEnd': Math.floor(
                  Date.now() / 1000
                ),
              },
            }
    ...
    if (
      !islandUpdateOperations.$push['islandResourceStats.claimableResources']
    ) {
      islandUpdateOperations.$push['islandResourceStats.claimableResources'] = {
        $each: [],
      };
    ...
    if (
      !islandUpdateOperations.$push['islandResourceStats.resourcesGathered']
    ) {
      islandUpdateOperations.$push['islandResourceStats.resourcesGathered'] = {
        $each: [],
      };
    ...
    while (!resourceToDrop) {
      resourceToDrop = randomizeResourceFromChances(
        <IslandType>island.type,
        island.traits,
        island.currentLevel
      );
    ...
    const claimableResources: ExtendedResource[] =
      island.islandResourceStats?.claimableResources;
    ...
    if (!claimableResources || claimableResources.length === 0) {
      ...
      const existingResourceIndex = claimableResources.findIndex(
        (r) => r.type === resourceToDrop.type
      );

      // if the resource already exists, increment its amount
      if (existingResourceIndex !== -1) {
        islandUpdateOperations.$inc[
          `islandResourceStats.claimableResources.${existingResourceIndex}.amount`
        ] = 1;
      } else {
    ...
    if (!resourcesGathered || resourcesGathered.length === 0) {
      ...
      const existingResourceIndex = resourcesGathered.findIndex(
        (r) => r.type === resourceToDrop.type
      );
      ...
      if (existingResourceIndex !== -1) {
        islandUpdateOperations.$inc[
          `islandResourceStats.resourcesGathered.${existingResourceIndex}.amount`
        ] = 1;
      } else {
        // if the resource doesn't exist, push a new resource
        const newResource: ExtendedResource = {
    ...
    if (<IslandType>island.type === IslandType.BARREN) {
      ...
      if (rand <= BARREN_ISLE_COMMON_DROP_CHANCE) {
        ...
        const commonResources = resources.filter(
          (r) => r.rarity === ResourceRarity.COMMON
        );
        const commonResourceToDrop =
        ...
        const existingResourceIndex = claimableResources.findIndex(
          (r) => r.type === commonResourceToDrop.type
        );
        ...
        const existingGatheredResourceIndex = resourcesGathered.findIndex(
          (r) => r.type === commonResourceToDrop.type
        );
        ...
        if (existingGatheredResourceIndex !== -1) {
          islandUpdateOperations.$inc[
            `islandResourceStats.resourcesGathered.${existingGatheredResourceIndex}.amount`
          ] = 1;
        } else {
          // if the resource doesn't exist, push a new resource
          const newResource: ExtendedResource = {
    ...
    if (
      (island.islandResourceStats?.dailyBonusResourcesGathered as number) <
      DAILY_BONUS_RESOURCES_GATHERABLE(<IslandType>island.type)
    ) {
      ...
      for (const bit of bits) {
        if (
          (bit.traits as BitTraitData[]).some(
            (trait) => trait.trait === BitTrait.LUCKY
          )
        ) {
        ...
        if (
          (bit.traits as BitTraitData[]).some(
            (trait) => trait.trait === BitTrait.UNLUCKY
          )
        ) {
          bonusResourceChance -= 2.5;
        }
        ...
        if (
          (bit.traits as BitTraitData[]).some(
            (trait) => trait.trait === BitTrait.TRICKSTER
          )
        ) {
        ...
        if (
          (bit.traits as BitTraitData[]).some(
            (trait) => trait.trait === BitTrait.HAPLESS
          )
        ) {
          bonusResourceChance -= 5;
        }
      ...
      console.log(
        `Island ${island.islandId} bonusResourceChance: ${bonusResourceChance}%`
      );
      ...
      if (bonusResourceChance > 0) {
        ...
        if (rand <= bonusResourceChance) {
          console.log(
            `(dropResource) rand is below bonusResourceChance. dropping bonus resource!`
          );
          ...
          // keep fetching a resource until it's not undefined (just in case it returns undefined at times)
          while (!bonusResource) {
            bonusResource = randomizeResourceFromChances(
              <IslandType>island.type,
              island.traits,
              island.currentLevel
            );
          ...
          console.log(
            `(dropResource) Island ${island.islandId} has dropped a bonus resource: ${bonusResource}`
          );
          ...
          const existingClaimableResourceToAddIndex =
            claimableResourcesToAdd.findIndex(
              (r) => r.type === bonusResource.type
            );
          const existingClaimableResourceIndex = claimableResources.findIndex(
            (r) => r.type === bonusResource.type
          );
          ...
          if (existingClaimableResourceIndex !== -1) {
            islandUpdateOperations.$inc[
              `islandResourceStats.claimableResources.${existingClaimableResourceIndex}.amount`
            ] = 1;
          } else if (existingClaimableResourceToAddIndex !== -1) {
            claimableResourcesToAdd[
              existingClaimableResourceToAddIndex
            ].amount += 1;
          } else {
            const newResource: ExtendedResource = {
              ...bonusResource,
          ...
          islandUpdateOperations.$inc[
            'islandResourceStats.dailyBonusResourcesGathered'
          ] = 1;
          ...
          const existingGatheredResourceToAddIndex =
            gatheredResourcesToAdd.findIndex(
              (r) => r.type === bonusResource.type
            );
          ...
          if (existingGatheredResourceIndex !== -1) {
            ...
            gatheredResourcesToAdd[
              existingGatheredResourceToAddIndex
            ].amount += 1;
          } else {
            const newResource: ExtendedResource = {
              ...bonusResource,
    ...
    islandUpdateOperations.$push[
      'islandResourceStats.claimableResources'
    ].$each.push(...claimableResourcesToAdd);
    islandUpdateOperations.$push[
      'islandResourceStats.resourcesGathered'
    ].$each.push(...gatheredResourcesToAdd);
...
export const randomizeResourceFromChances = (
  ...
  const effectiveDropChances: ResourceDropChance =
    calcEffectiveResourceDropChances(type, level);
  ...
  for (let [resourceRarity, probability] of Object.entries(
    effectiveDropChances
  )) {
    ...
    if (rand <= cumulativeProbability) {
      ...
      resourceRarity =
        resourceRarity.charAt(0).toUpperCase() + resourceRarity.slice(1);
      ...
      const resource = resources.find((r) => {
        ...
        if (trait === IslandTrait.MINERAL_RICH) {
          return (
            r.line === ResourceLine.ORE &&
            r.rarity === <ResourceRarity>resourceRarity
          );
        }

        if (trait === IslandTrait.AQUIFER) {
          return (
            r.line === ResourceLine.LIQUID &&
            r.rarity === <ResourceRarity>resourceRarity
          );
        }
      ...
      console.log(
        `(randomizeResourceFromChances) resource is undefined: `,
        resource === undefined
      );
      return resource;
    }
 ...
 */
export const addIslandToDatabase = async (
  island: Island
): Promise<ReturnValue> => {
  try {
    const newIsland = new IslandModel({
...
export const getLatestIslandId = async (): Promise<ReturnValue> => {
  try {
    ...
    const latestIsland = await IslandModel.findOne()
      .sort({ islandId: -1 })
      .lean();
...
export const calcIslandCurrentRate = (
  ...
  if (
    baseRates.length === bitLevels.length &&
    bitLevels.length === initialGrowthRates.length
  ) {
    // get the island rarity deviation multiplier
    const islandRarityDeviationMultiplier =
      ISLAND_RARITY_DEVIATION_MODIFIERS(islandType);
    ...
    for (let i = 0; i < n; i++) {
      ...
      const currentRate = calcBitCurrentRate(
        type,
        baseRates[i],
        bitLevels[i],
        initialGrowthRates[i],
        bitModifiers[i]
      );
    ...
    return (
      sum *
      (1 - reductionModifier * (n - 1)) *
      modifierMultiplier *
      islandRarityDeviationMultiplier
    );
  } else {
 ...
 */
export const calcEffectiveResourceDropChances = (
  type: IslandType,
  level: number
): ResourceDropChance => {
 ...
 */
export const calcResourceDropChanceDiff = (
  type: IslandType,
  level: number
): ResourceDropChanceDiff => {
 ...
 */
export const getIslandTappingData = async (
  islandId: number
): Promise<ReturnValue> => {
  try {
    const islandUpdateOperations = {
 ...
 */
export const applyIslandTapping = async (
  twitterId: string,
  islandId: number,
  caressMeter: number
): Promise<ReturnValue> => {
  try {
    ...
    const {
      caressEnergyMeter,
      currentCaressEnergyMeter,
      currentMilestone,
      milestoneReward,
    } = island.islandTappingData as IslandTappingData;
    const islandTappingLimit = ISLAND_TAPPING_MILESTONE_LIMIT(
      island.type as IslandType
    );
    ...
    console.log(
      `(applyIslandTapping), resources left for island ${island.islandId}: `,
      resourcesLeft
    );
    ...
    if (boosterPercentage < 100) {
      ...
      if (gatheringProgress + boosterPercentage > 100) {
        ...
        if (resourcesLeft === 0) {
          console.log(
            `(applyIslandTapping) Island ID ${islandId} has no resources left to drop. Cannot apply booster.`
          );
        ...
        if (status !== Status.SUCCESS) {
          console.log(
            `(applyIslandTapping) Error from dropResource: ${message}`
          );
      ...
      console.log(
        `(applyIslandTapping), gathering progress of island ${island.islandId}: `,
        gatheringProgress
      );
      console.log(
        `(applyIslandTapping), final non-modulo gathering progress of island ${island.islandId}: `,
        finalNonModuloGatheringProgress
      );
      ...
      if (resourcesToDrop > resourcesLeft) {
        console.log(
          `(applyIslandTapping) Island ID ${islandId} does not have enough resources left to drop. Cannot apply booster.`
        );
      ...
      for (let i = 0; i < resourcesToDrop; i++) {
        ...
        if (status !== Status.SUCCESS) {
          console.log(
            `(applyIslandTapping) Error from dropResource in loop: ${message}`
          );
        }
      }
    }
    ...
    userUpdateOperations.$set['inGameData.mastery.tapping.totalExp'] =
      newTotalExp;
    ...
    if (newTappingLevel > currentTappingLevel) {
      userUpdateOperations.$set['inGameData.mastery.tapping.level'] =
        newTappingLevel;
    }
    ...
    const energyRequired =
      Math.ceil(caressEnergyMeter / BASE_CARESS_PER_TAPPING) *
      BASE_ENERGY_PER_TAPPING;
    ...
    if (currentEnergy >= energyRequired) {
      ...
      userUpdateOperations.$set['inGameData.energy.currentEnergy'] =
        newCurrentEnergy;
      console.log(
        `(applyIslandTapping) deduct ${user.twitterUsername} energy to ${newCurrentEnergy} energy`
      );
    } else {
      return {
        status: Status.ERROR,
    ...
    if (currentMilestone <= islandTappingLimit) {
      const nextTappingData: IslandTappingData = ISLAND_TAPPING_REQUIREMENT(
        currentMilestone + 1
      );
...
export const resetDailyIslandTappingMilestone = async (): Promise<void> => {
  try {
    ...
    const islands = await IslandModel.find({
      'islandTappingData.currentMilestone': { $gt: 1 },
    }).lean();
