...
export const checkPOIItemDataResetAlreadyScheduled = async (timeRange: { start: number, end: number }): Promise<boolean> => {
    ...
    // If the current time is past the end of today's time range, schedule for tomorrow's time range
    console.log(`(checkPOIItemDataResetAlreadyScheduled), currentHour(${currentHour}) >= timeRange.end(${timeRange.end}) equals ${currentHour >= timeRange.end}`);
