...
//         const { status, message } = await handleTwitterLogin(twitterId, '123123', 'null');
...
router.get('/callback', passport.authenticate('twitter', { failureRedirect: '/', failureMessage: 'Error from callback.' }), async (req, res) => {
    if (!req.user) {
        return res.status(401).json({
            status: Status.UNAUTHORIZED,
            message: 'You denied the app or your session has expired. Please log in again.'
