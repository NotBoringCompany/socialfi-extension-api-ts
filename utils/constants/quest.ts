...
export const EXTRA_FACTORY_EXP_BENEFIT = (questType: QuestType, mastery: PlayerMastery, poiName: POIName): number => {
    ...
    switch(poiName) {
        case POIName.EVERGREEN_VILLAGE: return 1 * (level - 1);
        case POIName.PALMSHADE_VILLAGE: return 4 * (level - 1);
        case POIName.SEABREEZE_HARBOR: return 10 * (level - 1);
        default: return 0;
    }
};
 ...
 */
export const DAILY_QUEST_LEVEL_UP_REWARDS = ( level: number, poiName: POIName ): number => {
    switch(poiName) {
        case POIName.EVERGREEN_VILLAGE: {
            if (level >= 2 && level <= 4) {
                return 5;
