...
export const randomizeBitTraits = (rarity: BitRarity): BitTraitData[] => {
  ...
  const maxTraits =
    rarity === BitRarity.LEGENDARY
      ? 5
      : rarity === BitRarity.EPIC
      ? 4
      : rarity === BitRarity.RARE
      ? 3
      : 2;
  ...
  while (traits.length < maxTraits) {
    ...
    if (rand <= 80) {
      ...
      const commonTraits = bitTraits.filter(
        (trait) => trait.rarity === BitTraitRarity.COMMON
      );
      ...
      const uncommonTraits = bitTraits.filter(
        (trait) => trait.rarity === BitTraitRarity.UNCOMMON
      );
      ...
      const rareTraits = bitTraits.filter(
        (trait) => trait.rarity === BitTraitRarity.RARE
      );
    ...
    if (!traits.includes(randomTrait)) {
      ...
      if (categoryExists) {
        const existingSubCategory = traits.find(
          (trait) => trait.category === traitCategory
        )?.subcategory;
...
export const bitTraits: BitTraitData[] = [
  ...
  },
  {
    ...
    effect:
      "+5% working rate to all bits with same or lesser rarity in the same island",
    rarity: BitTraitRarity.UNCOMMON,
  ...
  },
  {
    ...
    effect:
      "+50% working rate to self, -5% working rate to all bits in the same island",
    rarity: BitTraitRarity.RARE,
    category: BitTraitCategory.WORKRATE_B,
    subcategory: BitTraitSubCategory.POSITIVE,
  },
 ...
 */
export const getBitStatsModifiersFromTraits = (
  traits: BitTrait[]
): BitStatsModifiers => {
  ...
  for (const traitEffect of traitEffects) {
    if (traitEffect.bitGatheringRate) {
      bitStatsModifiers.gatheringRateModifiers.push(
        traitEffect.bitGatheringRate
      );
    }
    ...
    if (traitEffect.energyDepletionRate) {
      bitStatsModifiers.energyRateModifiers.push(
        traitEffect.energyDepletionRate
      );
    }
    if (traitEffect.foodConsumptionEfficiency) {
      bitStatsModifiers.foodConsumptionEfficiencyModifiers.push(
        traitEffect.foodConsumptionEfficiency
      );
    }
  }
...
export const ENERGY_THRESHOLD_REDUCTIONS = (
  energy: number
): EnergyThresholdReduction => {
  switch (true) {
    case energy <= 0:
